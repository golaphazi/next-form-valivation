...
// active tab
		...
		if(forms_dat){
			...
			if(getType == 'next'){
				...
				if(tagName){
					...
					if(!tabIndex){
						// cal back function 
						forms_dat.submit(next_form_submit);
						return false;
					}
				}else{
					tabIndex = getIndex+'-0';
	...
	function __wfp_validateForm(formId, currentTab) {
			...
			if( y[i].hasAttribute("required") ){
			 ...
			 if( y[i].hasAttribute("wfp-pattern") ){
				 ...
				 if(patt == 'email'){
					 ...
					 var targetId = y[i].getAttribute("wfp-target-pattern");
					 if(targetId){
						 var targetIdData = formId.querySelector(targetId);
						 if(targetIdData){
							 if(!wfp_equal(y[i].value, targetIdData.value)){
								 y[i].classList.add("wfp-invalid");
								 return false;
							 }
						 }
					 }
				 }else if(patt == 'not-equal'){
					 var targetId = y[i].getAttribute("wfp-target-pattern");
					 if(targetId){
						 var targetIdData = formId.querySelector(targetId);
						 if(targetIdData){
							 if(!wfp_not_equal(y[i].value, targetIdData.value)){
								 y[i].classList.add("wfp-invalid");
								 return false;
							 }
						 }
	  ...
	  if (valid) {
		//document.querySelectorAll(".wfp-tab")[currentTab].classList.add("finish");
	  }
	  return valid; 
	}
...
function wfp_not_equal(str, target){
	if(str == target){
		return false;
	}
	return true;
